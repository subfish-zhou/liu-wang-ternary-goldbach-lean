...
最新实际主根见 [`docs/gaps/ROOT-INPUTS-20260915.md`](docs/gaps/ROOT-INPUTS-20260915.md)：主弧及第二、第三、第四弧已接入大数范围的条件性三素数出口，完整零密度及小导子排除已经证明并接入，只保留 **2 项共享数学输入**（有限高度 ζ 验证、全域 totient）。这不是无条件主定理。早期分批验收保留在 [`docs/gaps/ACCEPTED-20260915.md`](docs/gaps/ACCEPTED-20260915.md)；共同入口为 `LiuWang.Proof.Campaign20260915.Accepted`。
...
另一方面，当前主根中有限高度 ζ 验证、全域 totient 两项数学参数，即使不属于新声明的 `axiom`，也必须真正证明并接入，才可得到无条件结果。发布检查点约定见 [`docs/CHECKPOINTS.md`](docs/CHECKPOINTS.md)。
