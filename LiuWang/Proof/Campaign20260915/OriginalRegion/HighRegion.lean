...
/-! Original-c1 exclusion for every nonprincipal character above absolute height one half. -/
...
theorem primitive_high_original {x : ℝ} {q : ℕ} [NeZero q]
    ...
    (ht : 1 / 2 ≤ |rho.im|) : chi.LFunction rho ≠ 0 := by
  intro hz
  ...
  have hh := lecture_high_pole_free_budget hc hp (by linarith) hz ht
  ...
  have hpaid : 19.073344004352 / (1 + u - rho.re) ≤
    ...
    linarith
  exact lecture_scaled_budget_contradiction hL hb hgap.le hpaid

theorem ordinary_high_original {x : ℝ} {q : ℕ} [NeZero q]
    (hx : 10 ≤ x) (chi : DirichletCharacter ℂ q) (hc : chi ≠ 1)
