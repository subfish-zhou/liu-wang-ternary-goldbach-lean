import LiuWang.Proof.Campaign20260915.Density.SharpCriticalKernel
...
theorem source_family_equation_30_data (q : ℕ) [NeZero q] {y : ℝ}
  ...
  have haL : Real.log q ≤ Real.log ((q : ℝ) * y) := by
    rw [Real.log_mul hq0.ne' hy0.ne']
    linarith
  have hd : 0 ≤ sourceDelta y := by unfold sourceDelta; positivity
  have hB (u : ℝ) (hu : 0 ≤ u) : 0 ≤ sharpCriticalWindowBudget q y u := by
    have hR := chen24_rounded_bracket_nonneg (show 0 ≤ Real.log ((q : ℝ) * y) by linarith) haL hu
  ...
  have h := source_family_weighted_window q hylog
    ...
    hB (sharp_critical_kernel_integrable q y) (fun u hu => source_family_critical_sharp_window q hy hu)
  simp only [Complex.ofReal_div, Complex.ofReal_one, Complex.ofReal_ofNat] at h
  have hbound := h.2.trans (source_sharp_critical_kernel q hy)
  ...
  refine ⟨h.1, hbound.trans_eq ?_⟩
  rw [mul_assoc 235.69 _ (y ^ (1 / 2 : ℝ)), he]
  ring

theorem source_family_equation_30 (q : ℕ) [NeZero q] {y : ℝ}
    (hy : max (100000 / (q : ℝ)) (10000 * Real.log q) ≤ y) :
    (∑ chi ∈ nonprincipalCharacters q, ∫ t : ℝ,
      ‖cosineDetector chi ((q : ℝ) * y) y ((1 / 2 : ℂ) + t * I)‖ ^ 2) ≤
      235.69 * ((q : ℝ) * y) ^ (3 / 2 : ℝ) * y ^ (1 / 2 : ℝ) *
        Real.log ((q : ℝ) * y) ^ 2 :=
  (source_family_equation_30_data q hy).2

theorem source_cosineMean_critical_paid (q : ℕ) [NeZero q] {y : ℝ}
    (hy : max (100000 / (q : ℝ)) (10000 * Real.log q) ≤ y) :
    cosineMean q ((q : ℝ) * y) (1 / 2) y ≤
      (235.69 * ((q : ℝ) * y) ^ (3 / 2 : ℝ) * y ^ (1 / 2 : ℝ) *
        Real.log ((q : ℝ) * y) ^ 2) / q.totient := by
  unfold cosineMean
  simp only [Complex.ofReal_div, Complex.ofReal_one, Complex.ofReal_ofNat]
  exact div_le_div_of_nonneg_right (source_family_equation_30 q hy) (Nat.cast_nonneg _)
