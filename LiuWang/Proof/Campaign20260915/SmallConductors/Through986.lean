import LiuWang.Proof.Campaign20260915.SmallConductors.Through950
...
theorem primitive_le_986_original_region {q : ℕ} [NeZero q]
    (hq : q ≤ 986) (chi : DirichletCharacter ℂ q) (hp : chi.IsPrimitive)
    (hc : chi ≠ 1) (hsq : chi ^ 2 = 1) {x beta : ℝ}
    (hx : 10 ≤ x) (hqx : (q : ℝ) ≤ x)
    (hb : 1 - 1 / (9.645908801 * Real.log x) ≤ beta) :
    chi.LFunction (beta : ℂ) ≠ 0 := by
  by_cases hprev : q ≤ 950
  · exact primitive_le_950_original_region hprev chi hp hc hsq hx hqx hb
  · exact Fast.range_951_986 (by omega) hq chi hp hc hsq hqx hb

theorem original_source_conductor_ge_987 (d : PrimitiveQuadraticDatum)
    {x beta : ℝ} (hx : 10 ≤ x) (hdx : (d.modulus : ℝ) ≤ x)
    (hb : 1 - 1 / (9.645908801 * Real.log x) ≤ beta)
    (hz : datumL d (beta : ℂ) = 0) : 987 ≤ d.modulus := by
  let : NeZero d.modulus := ⟨d.modulus_ne⟩
  by_contra h
  exact primitive_le_986_original_region (by omega) d.character
    d.isPrimitive d.ne_one d.square_eq_one hx hdx hb hz

theorem actual_exceptional_conductor_ge_987 {N q : ℕ} [NeZero q]
    (hN : Real.exp 3100 ≤ (N : ℝ)) (hq : (q : ℝ) ≤ sourceP N)
    {chi : Character q} {rho : ℂ} (hr : rho ∈ exceptionalValues N chi) :
    987 ≤ chi.conductor := by
  classical
