import LiuWang.Proof.ZeroRegionFamily.ApplicationScale.Region

/-! # 固定一个实际本原来源，再量化全部模数及真实重数 -/
...
open LiuWang.Proof.ZeroRegionFamily.LowOrderClosure (region countedZeros real_sources_unique_fourteen)

namespace LiuWang.Proof.ZeroRegionFamily.ApplicationScale

theorem unique_datum_of_actual_zero {x : ℝ} {q : ℕ} [NeZero q]
    (hx : 20 ≤ Real.log x) (hq : (q : ℝ) ≤ x) (chi : DirichletCharacter ℂ q) {rho : ℂ}
  ...
  intro e he
  exact real_sources_unique_fourteen e _ (scale_ten_of_level hx hq) he.1 hd hpos hpos
...
theorem family_fixed_source {x : ℝ} {q : ℕ} [NeZero q]
  ...
  have he : primitiveSource chi hc hcs=primitiveSource psi hp hps :=
    real_sources_unique_fourteen _ _ (scale_ten_of_level hx hq) hd
  ...
  have hzero : tau=rho := by
    ...
    exact (primitive_real_zeros_unique
      ...
      (region_source_gap hx hq chi hc hwr) hgt).symm
...
theorem countedZeros_subsingleton {x : ℝ} {q : ℕ} [NeZero q]
    (hx : 20 ≤ Real.log x) (hq : (q : ℝ) ≤ x) :
    Subsingleton (LowOrderClosure.countedZeros x q) := by
  refine ⟨?_⟩
  rintro ⟨z, i⟩ ⟨w, j⟩
