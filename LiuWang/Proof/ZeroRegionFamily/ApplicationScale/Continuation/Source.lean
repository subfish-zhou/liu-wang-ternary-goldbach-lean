import LiuWang.Proof.ZeroRegionFamily.ApplicationScale.Continuation.Zeros

/-! # R40/3在全部x>=10的真实共同来源排斥 -/
...
theorem real_sources_unique (d e : PrimitiveQuadraticDatum)
    ...
    (hbg : 1-beta < 3/(40*Real.log x)) (hgg : 1-gamma < 3/(40*Real.log x)) : d=e := by
  by_contra hne
  ...
  have hterm (b : ℝ) (hb1 : b < 1) (hgap : 1-b < 3/(40*Real.log x)) :
      (40/13)*Real.log x < 1/(1+u-b) := by
    have hh := (lt_div_iff₀ (show 0 < 40*Real.log x by positivity)).mp hgap
