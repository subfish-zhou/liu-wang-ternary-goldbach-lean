import LiuWang.Proof.ZeroRegionFamily.ApplicationScale.Nonprincipal
...
theorem zeta_quartic_refined (p : RiemannXiDivisorZeroIndex)
    ...
    24480/(sigma-(riemannXiDivisorZeroValue p).re) ≤ 14379/(sigma-1)+
      46630*stechkinConductorCoeff*Real.log |(riemannXiDivisorZeroValue p).im|+2000 := by
  ...
  have hc := conductorCoeff_lower
  have hc1 := conductorCoeff_le_691_2500
  change 24480/(sigma-(riemannXiDivisorZeroValue p).re) ≤
    14379/(sigma-1)+46630*stechkinConductorCoeff*Real.log |t|+2000
  simp only [div_eq_mul_inv] at *
  nlinarith

theorem zeta_no_zero_fourteen {x : ℝ} (hx : 2 ≤ Real.log x) {rho : ℂ}
  ...
  have hh := zeta_quartic_refined p (by rw [hv]; linarith)
  ...
  have hupp : 24480/(1+u-rho.re) ≤
      (14379/(27/100)+46630*(691/2500)+1000)*Real.log x := by nlinarith only [hh, hl, hk, hx]
...
theorem principal_no_zero_fourteen {x : ℝ} {q : ℕ} [NeZero q]
  ...
  have hw : 1/(14*Real.log x) ≤ 1/2 := by
    ...
    linarith
  exact fun hz => zeta_no_zero_fourteen hx hr (ht.trans (div_le_self hx0 hq))
