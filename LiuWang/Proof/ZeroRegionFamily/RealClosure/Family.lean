import LiuWang.Proof.ZeroRegionFamily.RealClosure.Region

/-! # 同一x的真实共同本原来源、零点值与计重数单模唯一 -/
...
theorem family_zero_equal {x : ℝ} {q r : ℕ} [NeZero q] [NeZero r]
  ...
  have hzero := primitive_real_zeros_unique
  ...
  refine ⟨hzero, primitiveSource chi hc hcs, ?_, rfl, hdmod, chi.conductor_dvd_level, ?_,
...
theorem single_modulus_unique {x : ℝ} {q : ℕ} [NeZero q]
  ...
  obtain ⟨he, d, _, _, _, _, _, hdc, hdp, _⟩ :=
    family_zero_equal hx hq hq chi psi hwr hwt hpr hpt hzr hzt
...
theorem family_fixed_source {x : ℝ} {q : ℕ} [NeZero q]
  ...
  · intro r _ hr psi tau hwt hpt hzt
    ...
    have hzero := (family_zero_equal hx hq hr chi psi hwr hwt hpr hpt hzr hzt).1
    refine ⟨hzero.symm, congrArg PrimitiveQuadraticDatum.modulus hsource, ?_, ?_, hto⟩
    · rw [hsource]
      exact psi.conductor_dvd_level
    · rw [hsource]
      exact inducedAt_primitiveSource psi hp hps
