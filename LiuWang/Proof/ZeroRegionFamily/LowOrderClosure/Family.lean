import LiuWang.Proof.ZeroRegionFamily.LowOrderClosure.Region

/-! # R14同模真实重数至多一与一次固定的共同datum -/
...
theorem scale_ten_of_level {x : ℝ} {q : ℕ} [NeZero q]
    (hx : 160 ≤ Real.log x) (hq : (q : ℝ) ≤ x) : 10 ≤ x :=
  log_large_implies_ten ((show (0 : ℝ) < q by exact_mod_cast NeZero.pos q).trans_le hq) hx
...
theorem sources_equal {x : ℝ} {q r : ℕ} [NeZero q] [NeZero r]
    (hx : 160 ≤ Real.log x) (hq : (q : ℝ) ≤ x) (hr : (r : ℝ) ≤ x)
  ...
  exact real_sources_unique_fourteen _ _ (scale_ten_of_level hx hq)
    ((show (chi.conductor : ℝ) ≤ q by
      exact_mod_cast BombieriVinogradov.DirichletCharacter.conductor_le_level chi).trans hq)
...
theorem countedZeros_subsingleton {x : ℝ} {q : ℕ} [NeZero q]
    (hx : 160 ≤ Real.log x) (hq : (q : ℝ) ≤ x) : Subsingleton (countedZeros x q) := by
  refine ⟨?_⟩
  rintro ⟨z, i⟩ ⟨w, j⟩
