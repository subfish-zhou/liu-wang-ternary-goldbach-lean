import LiuWang.Proof.ZeroRegionFamily.LowOrderClosure.Source

/-! # R14全部普通L实单区域，闭高度与极点排除 -/
...
theorem region_re_half {x : ℝ} {q : ℕ} {rho : ℂ}
    (hx : 160 ≤ Real.log x) (hr : region x q rho) : 1/2 < rho.re := by
...
theorem zero_real_quadratic_simple {x : ℝ} {q : ℕ} [NeZero q]
  ...
  have hc : chi ≠ 1 := by
    intro he
    subst chi
    exact principal_no_zero_fourteen hx hr.1 hr.2 (hpole.resolve_left (by simp)) hz
  obtain ⟨hsq, hi⟩ := ordinary_zero_real_fourteen hx hq chi hc hr.1 hr.2 hz
