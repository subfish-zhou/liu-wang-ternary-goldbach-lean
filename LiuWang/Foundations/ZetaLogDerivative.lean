...
Authors: Terence Tao
Proof-only API backport for Lean 4.33.0-rc1 by the Liu–Wang foundations project.
Original theorem statement unchanged; source Mathlib revision 141f6b6455959bfeb0b2a6b04118031191d62683.
-/
module
...
theorem logDeriv_riemannZeta_one_sub {s : ℂ} (hs : ∀ n : ℤ, s ≠ n) (hz : riemannZeta s ≠ 0) :
  ...
  have hcoslog : logDeriv (fun z : ℂ ↦ cos (π * z / 2)) s =
      logDeriv cos (π * s / 2) * deriv (fun z : ℂ ↦ π * z / 2) s :=
    logDeriv_comp (f := cos) (g := fun z : ℂ ↦ π * z / 2) (by fun_prop)
      this.differentiableAt
  have hpowlog : logDeriv (fun z : ℂ ↦ (2 * (π : ℂ)) ^ (-z)) s = -log (2 * π) := by
    rw [logDeriv_apply, Complex.deriv_const_cpow (by fun_prop)]
    simp [neg_div, mul_div_cancel_right₀ _ (cpow_ne_zero_iff.mpr (Or.inl h2π))]
  ...
  have hlog : logDeriv (riemannZeta ∘ (1 - ·)) s =
      logDeriv (fun z : ℂ ↦
        2 * (2 * π) ^ (-z) * Gamma z * cos (π * z / 2) * riemannZeta z) s := by
    simp only [logDeriv_apply]
    rw [this.deriv_eq, this.self_of_nhds]
  have := hlog
  rw [logDeriv_comp, logDeriv_mul, logDeriv_mul, logDeriv_mul, logDeriv_mul,
    hcoslog, ← digamma_def] at this
